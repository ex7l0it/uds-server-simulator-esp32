...
/* AP Config */
WiFiAPClass WiFiAP;
const char *ssid = "ESP32_DoIP";
const char *password = "ESP32_DoIP_123456";
...
void loop()
{
  ...
  if (mode == 1)
  {
    ...
    if (udp_len)
    {
      ...
      DoIPFrame frame(buffer, udp_len);
      frame.debug_print();

      std::pair<uint8_t *, uint32_t> response = parse_doip_frame(frame);
      uint8_t *data = response.first;
      uint32_t data_len = response.second;
      if (data != nullptr)
      {
      ...
      {
        Serial.println("Error: parse_doip_frame returned a null pointer.");
      }
      free(data);
      // drop data
      udp.flush();
